resource "aws_lb" "nlb" {
  ...
  name               = "${substr(var.friendly_name_prefix,0,25)}-tfe-lb"
...
resource "aws_lb_target_group" "nlb_443" {
  ...
  name     = "${substr(var.friendly_name_prefix,0,17)}-tfe-nlb-tg-443"
  protocol = "TCP"
  port     = 443
  vpc_id   = var.vpc_id
...
resource "aws_lb_target_group" "nlb_8800" {
  ...
  name     = "${substr(var.friendly_name_prefix,0,17)}-tfe-lb-tg-8800"
  port     = 8800
  protocol = "TCP"
  vpc_id   = var.vpc_id
